...
def begin_training():
    ...
    dataset_train = pd.read_excel(datasetFile, sheet_name = '2016to2019')# full set
    ...
    for i in range(60,3733): #total of 60 arrays (process all variables * 60 since theres a time sequence). 9569 lines in dataset - 60 = 9536.
    ...
    bens_keras_rnn_model.add(Bidirectional(LSTM(units = 64, activation = 'tanh', return_sequences = True)))
    bens_keras_rnn_model.add(Dropout(0.15))

    bens_keras_rnn_model.add(Bidirectional(LSTM(units = 64, activation = 'tanh', return_sequences = True)))
    bens_keras_rnn_model.add(Dropout(0.25))

    bens_keras_rnn_model.add(Bidirectional(LSTM(units = 64, activation = 'tanh')))
    ...
    checkpoint_path = "bi_lstm_training_fosa_16_19/cp.ckpt"
    bens_keras_rnn_model.load_weights(checkpoint_path)
    
    #bens_keras_rnn_model.compile(optimizer = "adam", loss = 'mean_squared_error', metrics = ['accuracy'])
    ...
    tf.keras.utils.plot_model(bens_keras_rnn_model, to_file='diagram_bi_gen.png', show_shapes=True, show_layer_names=True)
    ...
    batch size = 32 - helps control parts that are loaded in directly, avoiding use of too much ram being used.
    """
    ...
    save_weight_path = "bi_lstm_training_fosa_16_19/cp.ckpt"
    save_weight_dir = os.path.dirname(save_weight_path)

    # Saving the weights of the model
    cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath = save_weight_path, save_weights_only=True, verbose = 1) # at the end of each training iteration (epoch) a weight file is generated for future training incase (power cut out previously in the house :/).
    ...
    bens_keras_rnn_model.fit(x_train, y_train, epochs = setEpochs, batch_size = setBatch)# recieves custom inputted values from user from functions
...
#def begin_concattraining(): <-- not sepearated yet.
    dataset_test = pd.read_excel('fatalOrSerious2010to2019.xlsx', sheet_name = '2020to2021') #First 20% as a test to see the rest of the missing 80% predicted this time.
...
#the end of the dataset (train one) is part of the data going in.
    ...
    X_test = []
    for i in range(60, 1079): #1625 - 60 = 1565
    ...
    plt.plot(real_set, color ='blue', label = 'Real 2020 North West Recorded Accidents')
    plt.plot(predicted_accidents, color = 'green', label = 'Predicted 2020 to 2021 North West Accidents')
    
    plt.title('2020 to 2021 North West Accident prediction BI_LSTM')
    plt.xlabel('Accident')
    plt.ylabel('Accident Fatal or Serious Adjusted')
    plt.legend()
    plt.annotate('Trained with 2016 to 2019 data', xy=(0.05, 0.95), xycoords='axes fraction')
    plt.grid()
    mixer.music.stop()
    plt.show()

    plt.title('2020 to 2021 North West Accidents')
    plt.hist(real_set, color = 'red')
    plt.xlabel('Accident Fatal or Serious Adjusted')
    plt.ylabel('Accident')
    plt.grid()
    plt.show()

    plt.title('2020 to 2021 North West Accident prediction BI_LSTM')
    plt.hist(predicted_accidents, color = 'green')
    plt.xlabel('Accident Fatal or Serious Adjusted')
    plt.ylabel('Accident')
    plt.grid()
    plt.show()
